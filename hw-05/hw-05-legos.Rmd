...
**Write your answer here:** In this sample, the three most common first names of purchasers are Jackson, Jacob, and Joseph.
...
**Write your answer here:** In this sample, the three most common themes of Lego sets purchased are Star Wars, Nexo Knights, and Gear.

3.  Among the most common theme of Lego sets purchased, what is the most common subtheme? The most common subtheme of these legos are the Skybound sets.
...
**Write your answer here:** I created the age_group variable. The distribution shows that the market for lego's grows as you get above the 18 and under range, stopping after you reach 51 and over.
...
lego_sales %>%
  group_by(age_group) %>%
  summarise(total_sets = sum(quantity)) %>%
  arrange(desc(total_sets))
```

*Think about: What do you need to sum? What do you need to group by?*

**Write your answer here:** The age group that has purchased the highest number of Lego sets is the 36-50 years old.
...
lego_sales %>%
  group_by(age_group) %>%
  summarise(total_spent = sum(quantity * us_price)) %>%
  arrange(desc(total_spent))
```

**Write your answer here:** The age group that has spent the most money on Legos is also the 36-50 years old group with almost 2000 more than the next highest.
...
lego_sales %>%
  group_by(theme) %>%
  summarise(total_revenue = sum(quantity * us_price)) %>%
  arrange(desc(total_revenue)) %>%
  slice(1:5)
```

**Write your answer here:** The theme that has made the most revenue for Lego is the Star Wars brand.
...
lego_sales %>%
  mutate(
    ...
    total_per_transaction = quantity * us_price
  ) %>%
  group_by(area_code) %>%
  summarise(total_spent = sum(total_per_transaction)) %>%
  arrange(desc(total_spent))
```

**Write your answer here:** The area code that has spent the most on legos is the 956 area code with 719.16.
...
Be sure to include your research quesiton, the variables you'll need to use to answer the question, the type of plot that will answer the question along with the code, and 2-3 sentences interpreting your results. 

What is the age distribution of Star Wars lego sets bought?
...
lego_sales %>%
  ...
  labs(
    title = "Age Distribution of Star Wars Lego Purchasers",
    x = "Age",
    y = "Number of Purchasers"
  )
```
There are a pretty close split of Star Wars purchases, rising the highest between ages 20-25, and 35-40.
