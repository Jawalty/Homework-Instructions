...
It provides details on a large list of airbnb's
...
Try binwidths of 25, 50, and 100. Which gives you the most useful view of the data? 25 gives you the most accurate view of the data
...
*Which binwidth did you choose and why?* 25 shows you the spread of the data with the most detail.
...
*Which layout did you choose and why?* I kept the original neighborhood grid, the others were a sloppy representation of the data.
...
edibnb %>%
  filter(neighbourhood %in% $neighbourhood) %>%
  ggplot(aes(x = price, y = neighborhood)) +
  geom_density_ridges() +
  labs(
    x = "price",
    y = "neighborhood",
    title = "Price distributions of the top 5 most expensive neighborhoods"
  )
```
...
- Which neighborhood has the highest typical prices? My graphs are having errors but I can see that New Town has the highest prices
- Which has the most variation (spread) in prices? Old town and New town have the highest
- Are there any notable patterns or outliers? Bruntsfield has a low count and low spread while Leith has an extremely high count and medium spreaad.
